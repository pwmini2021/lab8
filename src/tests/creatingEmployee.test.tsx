...
const server = setupServer(
    rest.post('*/employees', async (request: MockedRequest, response: ResponseComposition, ctx) => {
...
describe('AddEmployeeFormContainer error handling test', () => {
    const changeServerToThrowError = () => {
        server.use(
            rest.post('*/employees', (request: MockedRequest, response: ResponseComposition, ctx) => {
                return response(
                    ctx.status(500),
                    ctx.json({
                        errorCode: 1,
